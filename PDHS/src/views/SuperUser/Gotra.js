...
export default function Gotra() {
  ...
  useEffect(() => {		
	...
	{gotraArray.map( (d, index) => 
		<Grid align="left" key={"DOC1"+index} item xs={12} sm={6} md={3} lg={3} >
			<Box className={gClasses.boxStyle} borderColor="black" borderRadius={7} border={1} >
			<Typography>
			<span align="left" className={gClasses.patientInfo2}>{d.name+" "}</span>
			<span align="right">
	...
	return (
		...
		<Box className={gClasses.boxStyle} borderColor="black" borderRadius={7} border={1} >
			...
			{(isDrawerOpened === "EDIT") &&   
				...
				<br />
				{(rename) &&
					<VsRadioGroup  value={emurName} radioField="name" radioList={gotraArray}
						onChange={(event) => setEmurName(event.target.value)}		
					/>
				}
				{(!rename) &&
