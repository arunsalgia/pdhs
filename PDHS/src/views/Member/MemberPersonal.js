  ...
  useEffect(() => {	
	...
	<div>
		{(family || admin) &&
			<div align="right">
				...
				<VsButton name="Split Family" onClick={() => splitFamily("APPLYSPLIT") } />
	...
	{memberArray.map( (m, index) => {
		...
		<Grid key={"MEMGRID"+index} className={gClasses.noPadding} key={"SYM"+index} container justify="center" alignItems="center" >
			<Grid align="left" item xs={7} sm={7} md={3} lg={3} >
				<Typography>
				<span className={(index === 0) ? gClasses.patientInfo2Brown : gClasses.patientInfo2Blue }>{getMemberName(m)+" ("+dispAge(m.dob, m.gender)+")"}</span>
				{(isMobile()) &&
					<span align="left" data-for={"MEMBER"+m.mid} data-tip={myInfo} data-iscapture="true" >
			...
			<Grid align="center"  item xs={4} sm={4} md={1} lg={1} >
				{(m.mobile !== "") &&
					<Typography className={gClasses.patientInfo2}>{m.patientInfo2}{m.mobile}</Typography>
				}
				{(m.mobile1 !== "") &&
					<Typography className={gClasses.patientInfo2}>{m.patientInfo2}{m.mobile1}</Typography>
				}
			</Grid>
			{(!isMobile()) &&
			...
			<Grid align="right" item xs={1} sm={1} md={1} lg={1} >
				{(edit) &&
					<VsRadio checked={radioRecord === index} onClick={() => setRadioRecord(index)}  />
				}
			</Grid>
		</Grid>
		</Box>
	...
	return(
		<div>
		{memberArray.map( t =>
			<ReactTooltip key={"MEMBER"+t.mid} type="info" effect="float" id={"MEMBER"+t.mid} multiline={true}/>
		)}
		</div>
	)}
	...
	{((isDrawerOpened === "ADDPERSONAL") || (isDrawerOpened === "EDITPERSONAL")) &&
		...
		/>
		<br />
		<VsSelect inputProps={{className: gClasses.dateTimeNormal}} style={SELECTSTYLE} label="Blood Group" options={BLOODGROUP} value={emurAddr9} onChange={(event) => { setEmurAddr9(event.target.value); }} />
		<TextValidator required className={gClasses.vgSpacing}
			inputProps={{className: gClasses.dateTimeNormal}}
	...
	<div>
		<Typography align="center" className={gClasses.title}>{((isDrawerOpened === "EDITSPLIT") ? "Create" : "Apply") + " new family (Select members and Hod for new family)"}</Typography>
		<BlankArea />
